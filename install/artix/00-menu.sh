...
function mainmenu() {
    while true; do
        ...
        case "$choice" in
        "Install Packages")
            bash "$HOME"/dotfiles/install/artix/02-install.sh
            ;;
        "External Configurations")
            bash "$HOME"/dotfiles/install/artix/03-external-configurations.sh
            ;;
        "Dotfiles")
            bash "$HOME"/dotfiles/install/artix/04-dotfiles.sh
            ;;
        "Configurations In Place")
            bash "$HOME"/dotfiles/install/artix/05-configuration-in-place.sh
            ;;
        "Reboot")
            sudo reboot
            ;;
